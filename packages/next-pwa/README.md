...
![size](https://img.shields.io/bundlephobia/minzip/@opensourceframework/next-pwa.svg) ![dependencies](https://img.shields.io/librariesio/release/npm/@opensourceframework/next-pwa) ![downloads](https://img.shields.io/npm/dw/@opensourceframework/next-pwa.svg) ![license](https://img.shields.io/npm/l/@opensourceframework/next-pwa.svg)

👋 Share your awesome PWA project 👉 [here](https://github.com/shadowwalker/@opensourceframework/next-pwa/discussions/206)
...
- 📴 Completely offline support with fallbacks [example](https://github.com/shadowwalker/@opensourceframework/next-pwa/tree/master/examples/offline-fallback-v2) 🆕
...
- ☕ No custom server needed for Next.js 9+ [example](https://github.com/shadowwalker/@opensourceframework/next-pwa/tree/master/examples/next-9)
- 🔧 Handle PWA lifecycle events opt-in [example](https://github.com/shadowwalker/@opensourceframework/next-pwa/tree/master/examples/lifecycle)
- 📐 Custom worker to run extra code with code splitting and **typescript** support [example](https://github.com/shadowwalker/@opensourceframework/next-pwa/tree/master/examples/custom-ts-worker)
...
> **NOTE 1** - `@opensourceframework/next-pwa` version 2.0.0+ should only work with `next.js` 9.1+, and static files should only be served through `public` directory. This will make things simpler.
>
> **NOTE 2** - If you encounter error `TypeError: Cannot read property **'javascript' of undefined**` during build, [please consider upgrade to webpack5 in `next.config.js`](https://github.com/shadowwalker/@opensourceframework/next-pwa/issues/198#issuecomment-817205700).
...
> If you are new to `next.js` or `react.js` at all, you may want to first checkout [learn next.js](https://nextjs.org/learn/basics/create-nextjs-app) or [next.js document](https://nextjs.org/docs/getting-started). Then start from [a simple example](https://github.com/shadowwalker/@opensourceframework/next-pwa/tree/master/examples/next-9) or [progressive-web-app example in next.js repository](https://github.com/vercel/next.js/tree/canary/examples/progressive-web-app).

```bash
yarn add @opensourceframework/next-pwa
```
...
> The following setup has nothing to do with `@opensourceframework/next-pwa` plugin, and you probably have already set them up. If not, go ahead and set them up.
...
**[Use this example to see it in action](https://github.com/shadowwalker/@opensourceframework/next-pwa/tree/master/examples/offline-fallback-v2)**

`@opensourceframework/next-pwa` helps you precache those resources on the first load, then inject a fallback handler to `handlerDidError` plugin to all `runtimeCaching` configs, so that precached resources are served when fetch failed.
...
- register: boolean - whether to let this plugin register service worker for you
  - default to `true`
  - set to `false` when you want to handle register service worker yourself, this could be done in `componentDidMount` of your root app. you can consider the [register.js](https://github.com/shadowwalker/@opensourceframework/next-pwa/blob/master/register.js) as an example.
...
- cacheStartUrl - whether to cache start url
  - default: `true`
  - [discussion of use case to not cache start url at all](https://github.com/shadowwalker/@opensourceframework/next-pwa/pull/296#issuecomment-1094167025)
...
- cacheOnFrontEndNav - enable additional route cache when navigate between pages with `next/link` on front end. Checkout this [example](https://github.com/shadowwalker/@opensourceframework/next-pwa/tree/master/examples/cache-on-front-end-nav) for some context about why this is implemented.
  - default: `false`
...
- reloadOnOnline - changes the behaviour of the app when the device detects that it has gone back "online" and has a network connection. Indicate if the app should call `location.reload()` to refresh the app.
  - default: `true`
- customWorkerDir - customize the directory where `@opensourceframework/next-pwa` looks for a custom worker implementation to add to the service worker generated by workbox. For more information, check out the [custom worker example](https://github.com/shadowwalker/@opensourceframework/next-pwa/tree/master/examples/custom-ts-worker).
  - default: `worker`
...
`@opensourceframework/next-pwa` uses a default runtime [cache.js](https://github.com/shadowwalker/@opensourceframework/next-pwa/blob/master/cache.js)
...
1. [Common UX pattern to ask user to reload when new service worker is installed](https://github.com/shadowwalker/@opensourceframework/next-pwa/blob/master/examples/lifecycle/pages/index.js#L26-L38)
