 ...
 *
 * **CSP Compatibility:** The `media`, `swap`, and `swap-high` strategies inject inline `onload` event handlers
 * which may be blocked by Content Security Policy (CSP) directives that restrict inline event handlers.
 * For CSP-restricted environments, consider using `preload: 'js'` or `preload: 'body'` as alternatives,
 * or ensure your CSP includes appropriate `script-src` directives.
 *
...
export default class Critters {
  ...
  setupLinkPreload(link, href, media, style, document, preloadMode) {
    // Warn about CSP compatibility for strategies that use inline onload handlers
    if (preloadMode === 'media' || preloadMode === 'swap' || preloadMode === 'swap-high') {
      this.logger.warn(
        `CSP Warning: The '${preloadMode}' preload strategy injects inline onload event handlers ` +
        `which may be blocked by Content Security Policy (CSP) directives. ` +
        `Consider using 'preload: "js"' or 'preload: "body"' for CSP-restricted environments.`
      );
    }
